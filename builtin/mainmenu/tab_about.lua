...
return {
	...
	cbf_formspec = function(tabview, name, tabdata)
		...
		table.insert_all(credit_list, {
			...
			"",
			core.colorize("#ff0", fgettext("Minetest Core Developers"))
		})
		prepare_credits(credit_list, core_developers)
		table.insert_all(credit_list, {
			"",
			core.colorize("#ff0", fgettext("Minetest Core Team"))
		})
		prepare_credits(credit_list, core_team)
		table.insert_all(credit_list, {
			"",
			core.colorize("#ff0", fgettext("Minetest Active Contributors"))
		})
		prepare_credits(credit_list, active_contributors)
		table.insert_all(credit_list, {
			"",
			core.colorize("#ff0", fgettext("Minetest Previous Core Developers"))
		})
		prepare_credits(credit_list, previous_core_developers)
		table.insert_all(credit_list, {
			"",
			core.colorize("#ff0", fgettext("Minetest Previous Contributors"))
		})
		...
		if PLATFORM == "Android" then
			table.insert(fs, "button[0.5,6.1;4.5,0.8;share_debug;" .. fgettext("Share debug log") .. "]")
		end
