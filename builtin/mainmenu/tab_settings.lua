...
local function viewrange_to_id(vr)
	if vr <= 25 then
		return 1
	elseif vr <= 50 then
		return 2
...
local function formspec(tabview, name, tabdata)
	...
	if shaders_enabled then
		...
		if video_driver == "opengl" then
			...
			tab_string = tab_string ..
				"label[8.25,2.8;" .. fgettext("View range:") .. "]" ..
					"dropdown[8.25,3.25;3.5;dd_viewrange;25,50,100,150,200;" ..
					viewrange_to_id(tonumber(core.settings:get("viewing_range"))) .. "]"
		end
	else
		tab_string = tab_string ..
...
local function handle_settings_buttons(this, fields, tabname, tabdata)
	...
	if fields["dd_viewrange"] then
		core.settings:set("viewing_range", fields["dd_viewrange"])
		ddhandled = true
	end
