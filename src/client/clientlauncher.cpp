...
bool ClientLauncher::run(GameStartData &start_data, const Settings &cmd_args)
{
	...
	*/
	...
	skin->setColor(gui::EGDC_WINDOW_SYMBOL, video::SColor(200, 255, 255, 255));
		...
		const Settings &cmd_args)
{
	...
	if (cmd_args.exists("password-file")) {
		...
		if (passfile.good()) {
			...
			error_message = "Provided password file failed to open: "
	...
	if (start_data.address.empty()) {
		...
		if (worldspec.path.empty()) {
			error_message = "No world selected and no address provided. Nothing to do.";
			errorstream << error_message << std::endl;
			return false;
		}

		if (!fs::PathExists(worldspec.path)) {
			error_message = "Provided world path doesn't exist: "
					+ worldspec.path;
			errorstream << error_message << std::endl;
			return false;
		}
		...
		if (!start_data.game_spec.isValid()) {
			error_message = "Could not find or load game: "
					+ worldspec.gameid;
			errorstream << error_message << std::endl;
			return false;
		}
		...
		if (!start_data.game_spec.isValid()) {
			error_message = "Invalid gamespec.";
